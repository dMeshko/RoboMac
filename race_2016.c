...
void initialize()
{
 ...
 void calcIntegrals(int propertional, int *past_integrals, int *r1, int *up, int* sum, int iMax){
	 sum += propertional;
	 if (*r1 < iMax)
	 {
		 past_integrals[*r1++] = propertional;
	 }
	 else{
		 if (*up == iMax)
		 *up = 0;

		 sum -= past_integrals[*up];
		 past_integrals[*up++] = propertional;
	 }
 }
...
int main()
{
	...
	//derivative and integral values initially set to 0
	int derivative = 0;
	int integral = 0;
	int last_proportional = 0;
	const int iMaxMemory = 20;
	int r1 = 0;
	int up = 0;
	int sum = 0;
	int past_integrals[iMaxMemory];
	int lap = 0;
	const int tollerance = 50;
	const int max = 255; //255 is max
    while(1)
    {
		...
		if (sensors[0] >= 950 && sensors[1] >= 950 && sensors[2] >= 950 && sensors[3] >= 950 && sensors[4] >= 950){ //all dark
			if (lap == 0){
				lap = 1;
				set_motors(0, 0);
				delay_ms(50);
		...
		// position.
		calcIntegrals(proportional, past_integrals, r1, up, integral, iMaxMemory);
		...
		//int power_difference = proportional/10 + integral*100/1000 + derivative*7/3.33; // <- RABOTI so 200
		...
		int p = proportional * (max - 5) / 2000; //still needs to be tried out, dynamically adjust the speed
		
		if(power_difference - tollerance < 0) //turn left
			set_motors(max - p + power_difference, max - p);
		else if (power_difference + tollerance > 0) //turn right
			set_motors(max - p, max - p - power_difference);
		else //straight line
